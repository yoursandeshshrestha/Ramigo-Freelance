    ...
    <footer className="bg-brand-black">
      <div className="max-w-[1400px] mx-auto px-6 md:px-12 py-12 md:py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-12">
          ...
          {/* Services */}
          <div>
            <h3 className="font-trocchi text-lg text-brand-white mb-4">Services</h3>
            <nav className="flex flex-col gap-3">
              {navLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="font-hanken text-sm text-gray-400 hover:text-remigo-red transition-colors cursor-pointer"
                >
                  {link.label}
                </Link>
              ))}
          ...
          {/* Contact */}
          <div>
            <h3 className="font-trocchi text-lg text-brand-white mb-4">Contact</h3>
            <div className="flex flex-col gap-3">
              <a
                href="tel:+441234567890"
                className="font-hanken text-sm text-gray-400 hover:text-remigo-red transition-colors cursor-pointer flex items-center gap-2"
              >
            ...
            <div className="flex items-center gap-4 mt-6">
              {socialLinks.map((social) => (
                <a
                  ...
                  className="w-10 h-10 rounded-full bg-gray-800 flex items-center justify-center hover:bg-remigo-red transition-colors cursor-pointer"
          ...
          {/* Legal */}
          <div>
            <h3 className="font-trocchi text-lg text-brand-white mb-4">Legal</h3>
            <nav className="flex flex-col gap-3">
              {legalLinks.map((link) => (
                <Link
                  key={link.href}
        ...
        <div className="border-t border-gray-800 pt-8 mb-8">
          <div className="bg-gray-900/50 border border-gray-800 rounded-2xl p-6">
            <p className="font-hanken text-xs text-gray-400 leading-relaxed mb-4">
              <strong className="text-brand-white">Important:</strong> Your home may be repossessed if you do not keep up repayments on your mortgage.
            </p>
            <p className="font-hanken text-xs text-gray-400 leading-relaxed mb-4">
