...
export const QuestionnaireIcon = () => (
  <div className="w-full aspect-4/3 bg-[#0A0A0A] rounded-xl border border-gray-800/30 p-4 overflow-hidden relative">
    {/* Form header */}
    <div className="text-[8px] text-gray-600 font-medium mb-3">QUICK QUESTIONS</div>

    <div className="space-y-2.5">
      {/* Question 1 - Radio buttons */}
      <motion.div
        className="bg-[#111] border border-gray-800 rounded-lg p-2.5"
        initial={{ opacity: 0, y: 10 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.5, delay: 0.2, repeat: Infinity, repeatDelay: 4 }}
      >
        <div className="text-[9px] text-gray-400 font-medium mb-2">What are you looking for?</div>
        <div className="space-y-1.5">
          {['Remortgage', 'First home', 'Self-employed'].map((option, i) => (
            ...
            >
              <div className={`w-3 h-3 rounded-full border ${i === 0 ? 'border-brand-white/40 bg-brand-white/10' : 'border-gray-700'} flex items-center justify-center`}>
                {i === 0 && (
                  <motion.div
              ...
              <span className="text-[8px] text-gray-400">{option}</span>
            </motion.div>
          ))}
        </div>
      </motion.div>
      ...
      {/* Progress indicator */}
      <motion.div
        className="flex items-center justify-between bg-[#0D1117] border border-gray-700 rounded-lg p-2"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ duration: 0.4, delay: 1.2, repeat: Infinity, repeatDelay: 4 }}
      >
        <span className="text-[8px] text-gray-400">Question 1 of 5</span>
        <div className="flex gap-1">
          {[0, 1, 2, 3, 4].map((i) => (
            <motion.div
              ...
              animate={{ backgroundColor: i === 0 ? '#F8F8F8' : '#374151' }}
...
export const ComparisonIcon = () => (
  <div className="w-full aspect-4/3 bg-[#0A0A0A] rounded-xl border border-gray-800/30 p-4 overflow-hidden relative">
    ...
    <div className="flex items-center gap-2 mb-3">
      <div className="text-[8px] text-gray-600 font-medium">SEARCHING 90+ LENDERS</div>
      <motion.div
      ...
      {[
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="text-[9px] text-gray-400 font-medium">{lender.name}</span>
          </div>
          <div className="flex items-center gap-2">
            <span className={`text-[11px] font-bold ${lender.best ? 'text-gray-200' : 'text-gray-500'}`}>
              {lender.rate}
            </span>
            {lender.best && (
              <motion.div
                className="text-[7px] px-1.5 py-0.5 bg-green-500/10 text-green-400 rounded-full border border-green-500/20"
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
...
export const CallNotificationIcon = () => (
  <div className="w-full aspect-4/3 bg-[#0A0A0A] rounded-xl border border-gray-800/30 p-4 overflow-hidden relative">
    {/* Notification header */}
    <div className="text-[8px] text-gray-600 font-medium mb-3">EXPERT RESPONSE</div>

    {/* Incoming call card */}
    <motion.div
      className="bg-[#0D1117] border border-gray-700 rounded-lg p-3 relative overflow-hidden"
    ...
    >
      ...
      <div className="flex items-center gap-3 mb-3">
        <motion.div
          className="w-8 h-8 bg-gray-800 rounded-full border border-gray-700 relative"
        ...
        <div>
          <div className="text-[9px] text-gray-300 font-medium">Sarah - Mortgage Expert</div>
          <div className="text-[8px] text-gray-500">Calling you now...</div>
        </div>
      </div>

      {/* Call timer */}
      <motion.div
        className="flex items-center justify-center gap-2 bg-[#111] border border-gray-800 rounded-lg p-2"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ duration: 0.4, delay: 1.2, repeat: Infinity, repeatDelay: 4 }}
      >
        ...
        />
        <span className="text-[9px] text-gray-400 font-medium">00:00</span>
      </motion.div>
    </motion.div>

    {/* Response time badge */}
    <motion.div
      className="flex items-center justify-between bg-[#111] border border-gray-800 rounded-lg p-2 mt-2.5"
      initial={{ opacity: 0, y: 10 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.4, delay: 1.5, repeat: Infinity, repeatDelay: 4 }}
    >
      <span className="text-[8px] text-gray-500">Avg. response time</span>
      <span className="text-[9px] text-gray-300 font-medium">&lt; 1 hour</span>
    </motion.div>
  </div>
);
...
export const ProgressTrackerIcon = () => (
  <div className="w-full aspect-4/3 bg-[#0A0A0A] rounded-xl border border-gray-800/30 p-4 overflow-hidden relative">
    {/* Progress header */}
    <div className="text-[8px] text-gray-600 font-medium mb-3">YOUR PROGRESS</div>
        ...
        >
          ...
          <div className="relative">
            {step.status === 'complete' ? (
              ...
              <motion.div
                className="w-4 h-4 rounded-full bg-brand-white/10 border border-brand-white/30 flex items-center justify-center"
                animate={{ scale: [1, 1.1, 1] }}
                transition={{ duration: 1.5, repeat: Infinity }}
              >
                <motion.div
                  className="w-2 h-2 rounded-full bg-brand-white"
    ...
    <motion.div
      className="mt-3 bg-[#0D1117] border border-gray-700 rounded-lg p-2"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      transition={{ duration: 0.4, delay: 1.4, repeat: Infinity, repeatDelay: 4 }}
    >
      <div className="flex items-center justify-between mb-1.5">
        <span className="text-[8px] text-gray-400">Overall progress</span>
        <span className="text-[9px] text-gray-200 font-medium">75%</span>
      </div>
      <div className="h-1 bg-gray-800 rounded-full overflow-hidden">
        <motion.div
          className="h-full bg-brand-white/80 rounded-full"
          initial={{ width: '0%' }}
          animate={{ width: '75%' }}
